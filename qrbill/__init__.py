...
class QRBill:
    ...
    def __init__(
        ...
        if self.account[:2] not in IBAN_ALLOWED_COUNTRIES:
            raise ValueError("IBAN must start with: %s" % ", ".join(
                IBAN_ALLOWED_COUNTRIES))
    ...
    def draw_swiss_cross(self, dwg, grp, origin, size):
        ...
        group.add(
            dwg.polygon(points=[
                (18.3, 0.7), (1.6, 0.7), (0.7, 0.7),
                (0.7, 1.6), (0.7, 18.3), (0.7, 19.1),
                (1.6, 19.1), (18.3, 19.1), (19.1, 19.1),
                (19.1, 18.3), (19.1, 1.6), (19.1, 0.7)
            ], fill='black')
        )
        group.add(
    ...
    def draw_bill(self, dwg):
        ...
        if self.ref_number:
            ...
            grp.add(
                dwg.text(
                    format_ref_number(self),
                    (margin, mm(y_pos)), **self.font_info
                )
            )
            y_pos += line_space
        ...
        grp.add(
            dwg.text(
                self.label("Amount"),
                (add_mm(margin, mm(12)), mm(72)),
                **self.head_font_info
            )
        )
        grp.add(
            dwg.text(
                self.currency, (margin, mm(77)),
                **self.font_info
            )
        )
        if self.amount:
        ...
        grp.add(
            dwg.text(
                self.label("Acceptance point"),
                (add_mm(RECEIPT_WIDTH, margin * -1), mm(86)),
        ...
        if self.payment_line:
            grp.add(dwg.line(
                start=(mm(RECEIPT_WIDTH), 0), end=(mm(RECEIPT_WIDTH), mm(
                    BILL_HEIGHT)),
                stroke='black', stroke_dasharray='2 2', fill='none'
            ))
        ...
        if self.amount:
            ...
            self.draw_blank_rect(
                dwg, grp,
                x=add_mm(RECEIPT_WIDTH, margin, mm(12)), y=mm(75),
        ...
        if self.debtor:
            ...
            self.draw_blank_rect(
                dwg, grp,
                x=payment_detail_left, y=mm(y_pos),
