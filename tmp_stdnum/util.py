...
from tmp_stdnum.exceptions import *
...
def get_number_modules(base='tmp_stdnum'):
    ...
    with warnings.catch_warnings():
        warnings.filterwarnings('ignore', category=DeprecationWarning, module=r'tmp_stdnum\..*')
...
def get_cc_module(cc, name):
    ...
    if cc in ('in', 'is', 'if'):
        cc += '_'
    try:
        mod = __import__('tmp_stdnum.%s' % cc, globals(), locals(), [str(name)])
